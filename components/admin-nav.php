<nav class="pills">
    <a href="<?= ROOTPATH ?>/admin/general" class="btn <?= $page == 'general' ? 'active' : '' ?>"><?= lang('common.general') ?></a>
    <a href="<?= ROOTPATH ?>/admin/features" class="btn <?= $page == 'features' ? 'active' : '' ?>"><?= lang('admin.features') ?></a>
    <a href="<?= ROOTPATH ?>/admin/activities" class="btn <?= $page == 'activities' ? 'active' : '' ?>"><?= lang('common.activities') ?></a>
    <a href="<?= ROOTPATH ?>/admin/roles" class="btn <?= $page == 'roles' ? 'active' : '' ?>"><?= lang('common.roles') ?></a>
</nav>